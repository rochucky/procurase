...
export default class NovaSolicitacao extends React.Component<MyProps, MyState> {
  ...
  render() {
    return (
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS == "ios" ? "padding" : undefined}>
        ...
        <Header navigation={this.props.navigation} label="Nova Solicitação" back={true}/>
        <ScrollView style={styles.form} showsVerticalScrollIndicator={false}>
          ...
          {this.state.logradouro !=  '' ? (
            <View>
              ...
              {this.state.images.map((item: {uri: string, blob: Blob}, index: number) => {
                return (
                  <CustomImage uri={item.uri} style={styles.image} onDelete={this.handleImageDelete.bind(this, index)}/> 
                )
              })}
              </View>
  ...
  async save(){
    ...
    if(hasErrors != ''){
      ...
      const imgPromise = this.state.images.map( async (item: {uri: string, blob: Blob}, index: number) => {
        let date = new Date()
      ...
      .then(() => {
        this.jobs.add(this.data)
        .then((result: any) => {
          this.setState({loading: false});
          Alert.alert(
            'Solicitação criada',
            'Sua solicitação foi criada e estará disponível para propostas de profissionais',
            [
              {
                text: 'Ok',
                onPress: () => {
      ...
      [
        {
          ...
          onPress: () => {
            var images: any = [];
            this.state.images.map((item: any, ind: any) => {
              if(index != ind){
                images.push(item)
              }
            });
          ...
          if(json.erro){
            ...
            if(json.logradouro == ''){
              Alert.alert(
                ...
                'Não encontramos seu endereço, podemos usar a sua localização atual para encontrar, gostaria de fazer isso?',
                [
                  {
                    text: 'Sim',
                    onPress: async () => {
